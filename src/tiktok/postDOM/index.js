...
export const  work_post_DOM = async(job)=>{
    try {
        ...
        for(let j=0;j<10;j++){
            ...
            try {
                ...
                data = extractWebappVideoDetail(res.data)
                if(data.length!=0&&data.statusCode==0){
...
function extractWebappVideoDetail(html) {
const key = `"webapp.video-detail":`;
  ...
  const objectString = html.slice(braceStart, i + 1);

  try {
    return JSON.parse(objectString);
  } catch (e) {
    console.error("❌ Still not valid JSON");
    return null;
  }
}