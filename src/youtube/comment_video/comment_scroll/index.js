...
const logger = getLogger('comment_scroll')
const queueCommentYTB = new Queue(nameBull.YTBCommnent, redisLocal);

export const comment_scroll_video = async(job)=>{
    ...
    try {
        ...
        /////reply
        const reply_comment_continuation = data.onResponseReceivedEndpoints.find(t=>t.appendContinuationItemsAction).appendContinuationItemsAction.continuationItems
        if(reply_comment_continuation==undefined){
            console.log(data.onResponseReceivedEndpoints.find(t=>t.appendContinuationItemsAction).appendContinuationItemsAction)
        }
        ...
        }).filter(x=>x.continuation!=undefined)
        const data_reply = {
    ...
    // ✅ Lỗi của Axios
            console.error("Axios Error:CommentScroll", error.message);
        } else {
