...
import { workcommentbig } from './src/tiktok/comment/commentbig/comment.js';
import { workcommentreplybig } from './src/tiktok/comment/commentbig/commentreply.js';
import { bullCommentbig } from './src/tiktok/comment/commentbig/bullcommentbig.js';
export const run = async()=>{
    try {
        ...
        await bullCommentbig()
        queueCommentTTBig.process(20,async(job,done)=>{
            if(job.data.typeCrawl=="comment"){
                await workcommentbig(job)
            }
            if(job.data.typeCrawl=="reply"){
                await workcommentreplybig(job)
